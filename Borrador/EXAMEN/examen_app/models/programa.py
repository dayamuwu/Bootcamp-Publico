...
class Programas:
    ...
    def guardar_programa(cls,data):
        query="INSERT INTO programas(titulo,canal,lanzamiento,descripcion,usuarios_id) VALUES(%(titulo)s,%(canal)s,%(lanzamiento)s,%(descripcion)s,%(usuario_id)s);"
    ...
    def get_usuario_programa(cls):
        query = "SELECT * FROM programas as r JOIN usuarios as u ON r.usuarios_id = u.id;"
        results = connectToMySQL('Tv_db2').query_db(query)
        lista_usuario=[]
        for row_from_db in results:
            obj_programa=cls(row_from_db)
            obj_programa.on_usuarios.append(Usuarios(row_from_db))
